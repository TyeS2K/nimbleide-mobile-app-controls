		...
		initLib_rtfeditor1: function(e, itm) { 
			...
			if (typeof(CKEDITOR) != 'undefined') {
                ...
                CKEDITOR.config.height = 'calc(100% - ' + ((mode == 'normal') ? '90px)' : '60px)');
                ...
                if (mode == 'normal') {
                    ...
                    buttons = 'Templates,Save,NewPage,Source,ExportPdf,Preview,Print,Maximize,ShowBlocks,About,' + 
                        'Styles,Format,Font,BidiLtr,Table,HorizontalRule,Smiley,SpecialChar,PageBreak,Iframe,' + 
                        ...
                        'PasteText,Paste,Redo,PasteFromWord,Cut,FontSize,BGColor,Image';
                    plugins = "html5video,pastetext,pastefromgdocs,pastefromword,wsc,document," + 
                        "youtube,scayt,videodetector,sourcearea,base64image,";
                }
                // Apply Configuration
                CKEDITOR.config.toolbarGroups = toolbar;
                CKEDITOR.config.removeButtons = buttons;
                CKEDITOR.config.removePlugins = plugins;
